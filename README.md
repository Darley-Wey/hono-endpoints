...
The Documentation tab uses native JS/TS documentation for the route's HTTP method (`.get`, `.post`). Each Endpoints documentation target keeps smart pointers to the method reference and its original identifier. A cancellable asynchronous read resolves the method and passes its own call-site context to the registered native language providers, which generate the content and select the call signature. The plugin returns that content unchanged and resolves its PSI links through the native providers. Pending requests follow source edits and become invalid when their call is deleted. Editor requests with existing context stay with the IDE. The adapter uses public APIs, does not resolve handlers or reformat signatures, and performs no startup warming. No OpenAPI middleware is required. Route-level OpenAPI tags do not replace the route call's documentation target.
